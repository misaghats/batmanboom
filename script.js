...
// پارامترهای پویا (متناسب با سایز صفحه)
let GRAVITY, BIRD_WIDTH, BIRD_HEIGHT, PIPE_WIDTH, PIPE_GAP, PIPE_MIN_HEIGHT, PIPE_MAX_HEIGHT, BIRD_JUMP, PIPE_SPEED;

let bird, pipes, score, isGameOver, gameStarted, pipeSpeed;
...
batmanImage.src = "Batman.png";

// ریسپانسیو کردن canvas و پارامترها
...
// تنظیم پارامترهای بازی براساس سایز فعلی
function setGameParameters() {
    GRAVITY = canvas.height * 0.0012;
    BIRD_WIDTH = Math.max(canvas.width * 0.07, 30);
    BIRD_HEIGHT = BIRD_WIDTH;
    PIPE_WIDTH = Math.max(canvas.width * 0.07, 40);
    PIPE_GAP = canvas.height * 0.25;
    PIPE_MIN_HEIGHT = canvas.height * 0.15;
    PIPE_MAX_HEIGHT = canvas.height * 0.65;
    BIRD_JUMP = -canvas.height * 0.045;
    PIPE_SPEED = Math.max(canvas.width * 0.004, 2);
}
...
function movePipes() {
    pipes.forEach((pipe, i) => {
        ...
        if (pipe.top.x + PIPE_WIDTH < 0) {
            pipes.splice(i, 1);
            score++;
            if (score === 5) showMessage("Damn, you are good!");
            if (score === 10) showMessage("U are great!");
            else if (score === 15) showMessage("U are master");
            else if (score >= 20 && score <= 30) showFixedMessage("No more left sir!! U are the best");
        }
    });
}
...
// طراحی ستون‌ها با حالت سنبله‌ای
